      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <span className="text-sm font-semibold text-gray-900">₹{totalSpending.toFixed(0)}/month</span>
          </div>
        </div>
        <button
          onClick={handleAdd}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[indigo-600] focus:border-[indigo-600]"
          />
        </div>
        <select
          value={filterCategory}
          onChange={(e) => setFilterCategory(e.target.value)}
          className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[indigo-600] focus:border-[indigo-600]"
        >
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-base text-[gray-900] truncate">{sub.name}</h3>
                      <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-lg border ${categoryStyle}`}>
                        {sub.category}
                      </span>
                      {sub.status === 'active' && (
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium bg-green-50 text-green-700 border border-green-200 rounded-lg">
                    ...
                    {sub.next_billing_date && (
                      <div className={`flex items-center gap-2 text-sm ${
                        ...
                        {isUrgent && (
                          <span className="ml-1 px-2 py-0.5 bg-amber-600 text-white text-xs font-semibold rounded-lg">
